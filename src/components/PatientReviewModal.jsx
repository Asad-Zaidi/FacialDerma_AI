...
const PatientReviewModal = ({ open, onClose, loading, error, reviewData, currentUser = null }) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        await generatePdfReport(predictionData, treatmentSuggestions, userData, dermComment, true, reviewData.prediction?.imageUrl);
    };
    ...
    const handleSharePdf = async () => {
        ...
        const pdfBlob = await generatePdfReportBlob(predictionData, treatmentSuggestions, userData, dermComment, reviewData.prediction?.imageUrl);
        if (!pdfBlob) return;
