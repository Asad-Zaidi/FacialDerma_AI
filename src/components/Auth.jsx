...
import { useAuth } from '../contexts/AuthContext';
import { apiLogin, apiSignUp } from "../api/api";
import { useEmailValidator } from '../hooks/useEmailValidator'; // Assuming the hook is in this path; adjust as needed

const Auth = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const { login } = useAuth();
    const { validateEmail } = useEmailValidator(); // Integrate the hook
    ...
    const handleSignupSubmit = async (e) => {
        ...
        // Block emails with suspicious patterns
        if (/@(test|temp|1234|fake|mail|noreply|no-reply|example)\./i.test(formData.email)) {
            setMessage('Please use a real, non-temporary email address.');
            setMessageType('error');
            return;
        }
