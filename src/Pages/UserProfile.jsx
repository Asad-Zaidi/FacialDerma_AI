...
const UserProfile = () => {
    ...
    const handleRemoveImage = async () => {
        ...
        try {
            ...
            setLoading(false);
            setTimeout(() => setRemovingImage(false), 300); // Reset animation state
        }
    };
                    ...
                    <div className="space-y-5">
                        <div className="bg-gradient-to-br from-white via-blue-50 to-white border border-gray-200 rounded-xl shadow-xl p-5 text-center ">
                            <div className="relative inline-block mb-3">
                                ...
                                <div className={`relative transition-all duration-300 ease-in-out ${removingImage ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                                ...
                                {!selectedImage && patient.profileImage && (
                                    <button
                                        ...
                                        disabled={loading}
                                        className="absolute bottom-1 left-1 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-full p-2 cursor-pointer shadow-lg transition-all transform hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed"
                                        title="Remove profile picture"
                                    >
                                        {loading ? <AiOutlineLoading3Quarters className="animate-spin h-3.5 w-3.5" /> : <FaTrash className="h-3.5 w-3.5" />}
                                    </button>
                                )}
                                <input
